...
export type AllButtonsD = {
  id: number;
  name: string;
  buttonThreadcode: string;
};

export default function ButtonThreads() {
  ...
  console.log(allButtonsWithoutId, "allButtonsWithoutId");

  const addButton = async () => {
    const data = {
      name: "Super Admin Button Thread",
      ButtonThreads: allButtonsWithoutId,
    };

    await axios
      .post(`${ENDPOINT}/api/v1/Setups/superadmin_add-buttonthread`, data, {
        headers: {
      ...
      .then((res: any) => {
        ...
        setLoading(false);
        toast.success("Button Threads added successfully");
      })
      .catch((err: any) => {
        console.log({ err: err });
  ...
  return (
    <div>
      <h5 className="text-xl font-medium">Button Threads</h5>
