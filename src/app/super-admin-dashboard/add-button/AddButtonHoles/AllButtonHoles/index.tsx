...
export type AllButtonsD = {
  id: number;
  name: string;
  buttonHolecode: string;
};

export default function ButtonHoles() {
  ...
  const addButton = async () => {
    const data = {
      name: "SuperAdmin Button Hole",
      ButtonHoles: allButtonsWithoutId,
    };

    await axios
      .post(`${ENDPOINT}/api/v1/Setups/superadmin_add-buttonhole`, data, {
        headers: {
      ...
      .then((res: any) => {
        ...
        setLoading(false);
        toast.success("Button Holes added successfully");
      })
      .catch((err: any) => {
        console.log({ err: err });
  ...
  return (
    <div>
      <h5 className="text-xl font-medium">Button Holes</h5>
